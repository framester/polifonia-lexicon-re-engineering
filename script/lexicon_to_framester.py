...
import urllib
from wordnet_utils import get_framester_uri_from_wn_synset_id
...
def create_rdf(input_file, output_file, lang, logic):
    ...
    for index, row in df.iterrows():
        ...
        if wn_synset_id and not pd.isna(wn_synset_id):
            wn_synset = get_framester_uri_from_wn_synset_id(wn_synset_id)
            
            if wn_synset:
                g.add((synset, n_skos["closeMatch"], URIRef(wn_synset)))
